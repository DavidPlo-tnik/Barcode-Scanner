...
self.addEventListener('fetch', (e) => {
  ...
  // Never intercept GitHub API (recipe writes / sha lookups) — always live.
  if (url.hostname === 'api.github.com') return;

  // Recipe JSON from the repo: network-first so edits show up promptly.
  // Recipe images use unique filenames (immutable) so they fall through to
  // the cache-first branch below, which is what we want.
  if (url.hostname === 'raw.githubusercontent.com' && url.pathname.includes('/recipes/')) {
